...
def cadastro_medico(request):
    ...
    if is_medico(request.user):
        messages.add_message(request, constants.WARNIG, 'Você já é médico!')
        return redirect('/medicos/abrir_horario')

    if  request.method == 'GET':
        especialidades = Especialidades.objects.all()
        return render(request,'cadastro_medico.html', {'especialdades': especialidades, 'is_medico':is_medico(request.user)})
        ...
        cim = request.FLIES.get('cim')
        rg = request.FLIES.get('rg')
        foto = request.FLIES.get('foto')
...
def abrir_horario(request):
    ...
    if not is_medico(request.user):
        ...
        return redirect('/ususarios/sair')
...
def finalizar_consulta(request, id_consulta):
    ...
    if request.user != consulta.data_aberta.user:
        messages.add_message(request, constants.ERROR, 'Você não tem permissão para encerrar esta consulta.')
